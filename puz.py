    ...
    for y1 in range(0, height, SLICE_SIZE):
        for x1 in range(0, width, SLICE_SIZE):
            try:
                ...
                close_img_name = get_image_paths()[now_images]
                #close_img_name = './mini/60.jpg'
                paste_img = Image.open(close_img_name)
                paste_img = ImageOps.fit(paste_img, (SLICE_SIZE, SLICE_SIZE), Image.ANTIALIAS, 0, (0.7, 0.65))
                if(now_images > 526):
                    now_images = 0
                else:
                    now_images += 1
...
def resize_pic(in_name,size):
    ...
    img = ImageOps.fit(img, (size, size), Image.ANTIALIAS, 0, (0.7, 0.65))
    return img
